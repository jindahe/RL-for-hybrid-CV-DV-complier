...
class HighPrecisionQuantumPredictor:
    def __init__(self, csv_path='/Users/jinboyu/Documents/GitHub/RL-for-hybrid-CV-DV-complier/experiment/five/result_logic_[abcde].csv'):
    ...
    def _get_features(self, abcde_list):
        """
        提取特征：
        1. Order: 绝对值之和
        2. NZ_Count: 非零个数
        3. Std: 非零绝对值的标准差
        """
        if abcde_list is None: return None, None, None
        
        # 只提取非零元素的绝对值
        non_zeros = [abs(x) for x in abcde_list if x != 0]
    ...
    def load_and_train(self, csv_path):
        try:
            ...
            # 1. 提取所有特征 (从 K=[abcde] 列)
            features = df['K=[abcde]'].apply(lambda x: self._get_features(self._parse_abcde(x)))
